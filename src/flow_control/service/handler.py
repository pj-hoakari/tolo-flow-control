...
def handle_request(req: Request) -> Response:
    ...
    if early_verdict is not None:
        return _response(
            req,
            early_verdict,
            finalize_detection_state(detection, early_verdict, req),
            started,
    ...
    opt_cap = (
        req.config.lightweight_opt_budget_sec
        if req.config.optimization_mode.value == "LIGHTWEIGHT"
        else req.config.milp_time_limit_sec
    )
    cycle_budget = (
        req.config.forecasting_budget_sec + req.config.detour_budget_sec + opt_cap + BUFFER_SEC
    )
    if final_retry:
        cycle_budget *= 2
    ...
    forecast_result = forecast(
        ...
        req.history_digest,
        req.references,
        detection.triggered_edges,
        _forecast_config(req.config),
        mode,
    )
    ...
    optimize_result = optimize(
        ...
        req.history_digest,
        req.previous_result,
        _optimization_config(req.config),
        req.config.solver_seed,
        opt_budget,
        mode,
    ...
    state = _with_demand_digest(
        finalize_detection_state(detection, Verdict.OPTIMIZED, req), forecast_result
    )
    return _response(
        req,
        Verdict.OPTIMIZED,
...
def finalize_detection_state(
    det: DetectionResult, verdict: Verdict, req: Request
) -> DetectionState:
...
def _response(
    ...
    *,
    ...
    feedback: FeedbackValues | None = None,
    optimization_result: OptimizationResult | None = None,
) -> Response:
    return Response(
        request_id=req.request_id,
        verdict=verdict,
        updated_detection_state=state,
...
def _detection_warnings(det: DetectionResult) -> tuple[Warning, ...]:
    ...
    for item in det.warnings:
        ...
        result.append(
            Warning(code=code, context=(("edge_id", item.edge_id.value if item.edge_id else ""),))
        )
    return tuple(result)
...
def _validation_errors(req: Request) -> tuple[str, ...]:
    errors: list[str] = []
    if (
        req.schema_version != REQUEST_SCHEMA_VERSION
        or not req.request_id
        or not req.tenant_context.tenant_id
    ):
    ...
    if any(
        edge.endpoint_a not in node_set or edge.endpoint_b not in node_set
        for edge in req.graph.edges
    ):
        errors.append("edge endpoint")
    cfg = req.config
    positive = (
        cfg.forecasting_budget_sec,
        cfg.detour_budget_sec,
        cfg.lightweight_opt_budget_sec,
        cfg.milp_time_limit_sec,
        cfg.puncture_ratio_threshold,
    )
    if (
        cfg.cooldown_duration_min < 10
        or any(not math.isfinite(value) or value <= 0 for value in positive)
        or cfg.max_consecutive_skips < 1
        or not 0 <= cfg.confidence_weight_floor <= 1
        or cfg.k_shortest_paths < 1
        or cfg.local_radius_hops < 1
        or cfg.max_trigger_zones < 1
...
def _detection_config(cfg: ServiceConfig) -> DetectionConfig:
    return DetectionConfig(
        ...
        high_stagnation_duration_min=cfg.high_stagnation_duration_min,
        beta=cfg.beta,
        theta_demand=cfg.theta_demand,
        cooldown_duration_min=cfg.cooldown_duration_min,
        queue_score_threshold=cfg.queue_score_threshold,
        queue_diversity_threshold=cfg.queue_diversity_threshold,
        queue_freshness_min=cfg.queue_freshness_min,
        warmup_duration_min=cfg.warmup_duration_min,
...
def _forecast_config(cfg: ServiceConfig) -> ForecastConfig:
    return ForecastConfig(
        min_reference_sample_count=cfg.min_reference_sample_count,
        fallback_eta=cfg.fallback_eta,
        gravity_alpha=cfg.gravity_alpha,
        delta_min=cfg.delta_min,
        transit_time_prior_sec=cfg.transit_time_prior_sec,
        dwell_time_prior_sec=cfg.dwell_time_prior_sec,
        ipf_max_iter=cfg.ipf_max_iter,
        ipf_tolerance=cfg.ipf_tolerance,
        epsilon_0=cfg.epsilon_0,
    )
...
def _optimization_config(cfg: ServiceConfig) -> OptimizationConfig:
    return OptimizationConfig(
        optimization_mode=ModuleOptimizationMode(cfg.optimization_mode.value),
        local_radius_hops=cfg.local_radius_hops,
        max_trigger_zones=cfg.max_trigger_zones,
        greedy_improve_margin=cfg.greedy_improve_margin,
        lightweight_opt_budget_sec=cfg.lightweight_opt_budget_sec,
        restriction_proposal_enabled=cfg.restriction_proposal_enabled,
        tau_danger_threshold=cfg.tau_danger_threshold,
        solver_seed=cfg.solver_seed,
        milp_time_limit_sec=cfg.milp_time_limit_sec,
        epsilon=cfg.epsilon,
        epsilon_0=cfg.epsilon_0,
        big_m_factor=cfg.big_m_factor,
        delta_min=cfg.delta_min,
        confidence_weight_floor=cfg.confidence_weight_floor,
        throughput_target_edges=cfg.throughput_target_edges,
        fallback_baseline_stagnation=cfg.fallback_baseline_stagnation,
