...
既知問題: 現状の固定方向 LP は非循環フラグがバイナリのまま残るため実態は MIP であり、
本規模では HiGHS の MIP ヒューリスティクス内部でヒープ破壊が起き
プロセスごと SIGABRT で落ちる（docs/performance_evaluation_20260720.md の P2/P3）。
run-all に含めると index が書かれないため、計測は単独 run を推奨。
"""
