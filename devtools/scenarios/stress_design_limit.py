...
かつて固定方向 LP に残っていたバイナリ z により実態が MIP となり、本規模では
HiGHS の MIP ヒューリスティクス内部のヒープ破壊で SIGABRT していた（レポート P2/P3）。
配分 LP のバイナリゼロ化（A1、2026-07-20）でクラッシュは解消し予算 120s 内で完走する。
ただし所要 1 分超と重いため、日常の run-all からは引き続き除外し単独 run で計測する
（モデル構築の高速化 A5 後に再検討）。
"""
...
def build() -> Scenario:
    ...
    return make_scenario(
        ...
        # 既知問題（HiGHS MIP でのプロセス異常終了）により run-all から除外。単独 run で計測する
